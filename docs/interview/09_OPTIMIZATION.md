...
| 默认日志目录硬编码 `<workspace>\logs`（`serial_log_receiver.py:25`） | 参数默认值改为相对路径/环境变量 |
...
| 现状（证据） | 优化方向 |
| --- | --- |
| 硬编码 `<workspace>`（所有 bat/ps1） | 脚本顶部 `%~dp0` 定位自身目录，或环境变量 `%JL_ROOT%` |
...
| 现状（证据） | 优化方向 |
| --- | --- |
| `sys.path.insert(0, r"<workspace>\python_pkgs")`（第 6 行） | 相对路径/虚拟环境/`--pkgs` 参数 |
...
> P1：去掉硬编码路径（`<workspace>` → 环境变量/相对路径）；
> P2：GUI 环形缓冲 + 滚动开关（体验）；
