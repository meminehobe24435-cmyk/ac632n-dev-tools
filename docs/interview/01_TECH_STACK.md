...
**为什么用**：杰理官方工具链是 Windows 优先（编译链在 `<sdk_path>\pi32`、`<sdk_path>\mc`，下载工具 `isd_download.exe`），且团队/同事习惯双击运行；比要求所有人手敲 `make` 命令更防错。
**怎么用**：
```bat
@echo off
setlocal
set SDK_DIR=<workspace>\sdk\fw-AC63_BT_SDK
set PATH=%SDK_DIR%\tools\utils;<sdk_path>\pi32\bin;<sdk_path>\mc\bin;%PATH%
cd /d "%SDK_DIR%"
...
**面试官怎么追问**：
- "为什么用 `sys.path.insert(0, r"<workspace>\python_pkgs")`？" → 把离线依赖目录插到 import 路径，脱离 pip 环境也能跑（但硬编码绝对路径，见 `13_CODE_REVIEW.md`）。
...
**是什么**：厂商编译链 `<sdk_path>\pi32`（含 `clang`、`make`）、`<sdk_path>\mc`；下载工具 `isd_download.exe` + `download.bat`；烧录走 USB UBOOT（BD19 UBOOT 模式）。
