...
| 难点 | 说明 | 证据 |
| --- | --- | --- |
| 厂商工具链 + Windows 构建闭环 | 杰理编译链在 `C:\JL\pi32`、`C:\JL\mc`，用 `make ac632n_spp_and_le` 触发，post-build 会自动拉起下载器 | `README_AC6321A_WORKFLOW.txt:20-23`、`build_ac632n_spp_and_le.bat` |
...
1. **构建链**：`make ac632n_spp_and_le` → `C:\JL\pi32` 编译 → 产物 `cpu/bd19/tools/download/data_trans/update.ufw` / `jl_isd.fw` → `download.bat`（USB UBOOT）烧录。
