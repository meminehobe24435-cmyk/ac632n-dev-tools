...
- 编译链：`C:\JL\pi32`（含 `clang`/`make`）、`C:\JL\mc`。
- 已确认本机存在（`C:\JL` 下含 `pi32`、`mc`、`commtools`、`isd_download` 相关）。
- `env_check/show_env_check.bat:3` 把 `C:\JL\pi32\bin;C:\JL\mc\bin` 加入 PATH 后 `where clang/make` 自检。

**面试口径**：
> 杰理工具链装在 `C:\JL\pi32`（clang 前端 + pi32 后端）和 `C:\JL\mc`，我写了个 `env_check/show_env_check.bat` 一键自检 codeblocks/clang/make/python/串口环境。
