...
static void wtyi_force_bt_name(void)
{
    memset(bt_cfg.edr_name, 0x00, LOCAL_NAME_LEN);
    memcpy(bt_cfg.edr_name, WTYI_BT_CLASSIC_NAME, sizeof(WTYI_BT_CLASSIC_NAME));
}
...
void cfg_file_parse(u8 idx)
{
    ...
    if ((ret != (sizeof(WTYI_BT_CLASSIC_NAME) - 1)) ||
        memcmp(tmp, WTYI_BT_CLASSIC_NAME, sizeof(WTYI_BT_CLASSIC_NAME) - 1)) {
        syscfg_write(CFG_BT_NAME, WTYI_BT_CLASSIC_NAME, sizeof(WTYI_BT_CLASSIC_NAME) - 1);
    }
    wtyi_force_bt_name();
    printf("[WTYI_BT] classic_name=%s\n", bt_cfg.edr_name);
