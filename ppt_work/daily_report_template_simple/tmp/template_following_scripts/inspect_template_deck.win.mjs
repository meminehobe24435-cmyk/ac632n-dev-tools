...
function runCapture(command, args, options = {}) {
  const spawnCommand = command === "unzip.cmd" ? "powershell.exe" : command;
  const spawnArgs = command === "unzip.cmd"
    ? [
        "-NoProfile",
        "-ExecutionPolicy",
        "Bypass",
        "-File",
        "D:/23178/JL/ppt_work/daily_report_template_simple/tmp/bin/unzip-shim.ps1",
        "-Mode",
...
function zipNames(pptxPath) {
  return String(runCapture("unzip.cmd", ["-Z1", pptxPath], { encoding: "utf8" }))
    .split(/\r?\n/)
    .filter(Boolean);
}

function readZipText(pptxPath, entryName) {
  return Buffer.from(runCapture("unzip.cmd", ["-p", pptxPath, entryName])).toString("utf8");
}

async function copyZipEntry(pptxPath, entryName, targetPath) {
  await fs.mkdir(path.dirname(targetPath), { recursive: true });
  await fs.writeFile(targetPath, Buffer.from(runCapture("unzip.cmd", ["-p", pptxPath, entryName])));
}
